...
- Name: FileUpload Component E2E Tests >> FileUpload Component Rendering >> should render file upload component when airline is selected
- Location: C:\Users\ariel\Documents\GitHub\supabase-nextjs-template\tests\e2e\file-upload.spec.ts:131:9
...
Call log:
  - waiting for locator('[data-testid="airline-selector"]')
    ...
    at C:\Users\ariel\Documents\GitHub\supabase-nextjs-template\tests\e2e\file-upload.spec.ts:133:18
```
...
```ts
   ...
   42 | 0000000009 00000 n 
   43 | 0000000058 00000 n 
   44 | 0000000115 00000 n 
   45 | trailer
   46 | <<
   47 | /Size 4
   48 | /Root 1 0 R
   ...
   52 | %%EOF`;
   53 |
   54 |     fs.writeFileSync(path.join(testFilesDir, 'test-invoice.pdf'), validPDFContent);
   55 |     
   56 |     // Create a large test file (>25MB)
   57 |     const largeContent = 'A'.repeat(26 * 1024 * 1024); // 26MB
   58 |     fs.writeFileSync(path.join(testFilesDir, 'large-file.pdf'), largeContent);
   59 |     
   60 |     // Create a non-PDF file
   61 |     fs.writeFileSync(path.join(testFilesDir, 'test-document.txt'), 'This is a text file');
   62 |   });
   63 |
